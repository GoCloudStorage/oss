...
func convertContentRange(d string) (res model.ContentRange, err error) {
	t := strings.Split(d, " ")
	if len(t) != 2 {
...
func parasUploadHeader(c *fiber.Ctx) (*model.UploadReq, error) {
...
func parasUploadCommonHeader(c *fiber.Ctx) (*model.UploadReq, error) {
	// 获取Content-Range
	...
	return &model.UploadReq{ContentRange: cr, TotalSize: total, MD5: md5, Key: key}, nil
}
